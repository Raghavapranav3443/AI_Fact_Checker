...
# UUID v4 pattern — validates session_id path params to prevent injection
_UUID_RE = re.compile(
...
class IngestRequest(BaseModel):
    ...
    def validate_content(cls, v):
        ...
        # Hard cap at 60KB of raw content (10k words ≈ 60KB)
...
@router.post("/ingest")
@limiter.limit("10/minute;30/hour")   # per-IP: 10 req/min, 30 req/hour
async def ingest(request: Request, req: IngestRequest, background_tasks: BackgroundTasks):
    ...
    if intent["category"] in ("INSUFFICIENT_FACTS", "OFF_TOPIC"):
        raise HTTPException(
            status_code=400,
            detail=f"Content rejected ({intent['category']}): {intent.get('reason', 'Text does not contain verifiable factual claims.')}"
        )
    ...
    return {
        "session_id": session_id,
        "word_count": meta["word_count"],
        "estimated_time_seconds": meta["estimated_time_seconds"],
        "opinion_flag": opinion_flag,
        "status": "ok",
    }
...
async def stream(
    ...
    # Parse Last-Event-ID header (standard SSE reconnect protocol)
    ...
    return StreamingResponse(
        sse_stream(session_id, last_event_index=last_event_id),
        media_type="text/event-stream",
        headers={
            "Cache-Control":     "no-cache, no-store",
            "X-Accel-Buffering": "no",      # nginx: disable buffering
            "Connection":        "keep-alive",
            "Access-Control-Allow-Origin": "*",   # SSE needs open CORS
        },
    )
...
async def get_report(request: Request, session_id: str):
    ...
    if session.get("status") == "error":
        raise HTTPException(
            status_code=500,
            detail={"message": "Pipeline failed", "errors": session.get("errors", [])}
        )
    report = session.get("report", {})
    # Never return the full input_text in report endpoint — can be large
    safe_report = {k: v for k, v in report.items() if k != "input_text"}
